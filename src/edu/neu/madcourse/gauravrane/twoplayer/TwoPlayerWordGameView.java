...
import edu.neu.madcourse.gauravrane.twoplayer.TwoPlayerWordGame.Coordinate;
...
public class TwoPlayerWordGameView extends View {
	...
	private final TwoPlayerWordGame twoPlayerWordGame;
	...
	public TwoPlayerWordGameView(Context context) {
		super(context);
		this.twoPlayerWordGame = (TwoPlayerWordGame) context;
		setFocusable(true);
		setFocusableInTouchMode(true);
		setId(ID);
	}

	public TwoPlayerWordGameView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.twoPlayerWordGame = (TwoPlayerWordGame) context;
	}

	public TwoPlayerWordGameView(Context context, AttributeSet attrs, int defStyle) {
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (!isCoodInDictionary(i, j)) {
					canvas.drawText(Character.toString(twoPlayerWordGame.getRectLetter(i,j)),i * width + x, j * height + y, foreground);
				}
	...
	private boolean isCoodSelected(int i, int j) {
		return twoPlayerWordGame.isCoodInList(i, j);
	}

	private boolean isCoodInDictionary(int i, int j) {
		return twoPlayerWordGame.isCoodInMatch(i, j);
	}
	...
	public void invalidateNewChars(ArrayList<Coordinate> list){
		for(int i=0;i<list.size();i++){
			...
			Rect in = new Rect();
			getRect(coodX,coodY,in);
			twoPlayerWordGame.setRectLetter(coodX,coodY, twoPlayerWordGame.getCharForTile(new Random().nextInt(RANDOM_INDEX.length)));
			invalidate(in);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if(!twoPlayerWordGame.isPausedPressed){
			TwoPlayerMusic.playThird(twoPlayerWordGame,R.raw.dictionary_beep);
			...
			if(!isCoodSelected(eventX, eventY) && !isCoodInDictionary(eventX, eventY)){
				twoPlayerWordGame.addCood(eventX, eventY);
				selectRect(eventX, eventY);
				twoPlayerWordGame.setButtonText(eventX, eventY);
			}
		}
